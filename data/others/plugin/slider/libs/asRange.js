    ...
    _createClass(Pointer, [
      ...
      },
      {
        ...
        value: function getPercent() {
          ...
          ** Ogihara's edit
          */
          //return 5 + ((this.value - this.parent.min) / this.parent.interval * 100) * 0.91;
        ...
        },
        {
          ...
          value: function getValueFromPosition(px) {
            if (px > 0) {
              ...
              ** Ogihara's edit
              */
              var val = this.min + px / this.getLength() * this.interval;
              var config = TYRANO.kag.config;
              var view = $.getViewPort();
              var configWidth = parseInt(config.scWidth);
              var currentWidth =
                (config.ScreenRatio == "default") ? parseInt(config.scWidth):
                (config.ScreenRatio == "fit"    ) ? view.width:
                (view.height / view.width > config.scHeight / config.scWidth) ? view.width:
                config.scWidth * view.height / config.scHeight;
              var rate = configWidth / currentWidth;
              return val * rate;
            }
            return 0;
          }
