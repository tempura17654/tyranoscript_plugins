...
(function (window, document, exports, global, define, undefined) {
	...
	function renderDocument(document, options, windowWidth, windowHeight, html2canvasIndex) {
		//======以下を追加========
		windowWidth = TYRANO.kag.config.scWidth;
		windowHeight = TYRANO.kag.config.scHeight;
		//======追加はここまで========
	...
	CanvasRenderer.prototype.renderBackgroundGradient = function (gradientImage, bounds) {
		//======以下を追加========
		var getStops = function (str) {
			// 例）str = "-webkit-gradient(linear, 0% 0%, 0% 100%, from(rgb(178, 178, 237)), color-stop(0.49, rgb(80, 80, 165)), color-stop(0.5, rgb(28, 28, 96)), to(rgb(5, 5, 25)))";
			var head = str.indexOf("from(");
			var last = str.length - 1;
			str = str.substring(head, last) + ", "
			...
			for (i = 0, j = 0; i < len; i++) {
				...
				else if (s.indexOf("color-stop(") > -1) {
					s = s.replace("color-stop(", "");
					p = s.indexOf(", ");
					pos = parseFloat(s.substring(0, p));
					col = s.substring(p + 2, s.length);
				};
		...
		if (gradientImage instanceof WebkitGradientContainer) {
			var src      = gradientImage.src;
			var split    = src.split(",");
			var pos_from = split[1].split(" ");
			var pos_to   = split[2].split(" ");
			gradientImage.x0 = parseInt(pos_from[1]) / 100;
			gradientImage.y0 = parseInt(pos_from[2]) / 100;
			gradientImage.x1 = parseInt(pos_to[1]) / 100;
			gradientImage.y1 = parseInt(pos_to[2]) / 100;
			gradientImage.colorStops = getStops(src);
			...
			for (var g, i = 0; i < gradientImage.colorStops.length; i++ ) {
				g = gradientImage.colorStops[i];
